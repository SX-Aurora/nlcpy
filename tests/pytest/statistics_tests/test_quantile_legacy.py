...
class TestQuantile(unittest.TestCase):
    ...
    def test_me_case_3(self, xp):
        a = xp.array([[10, 7, 4], [3, 2, 1]])
        return xp.quantile(a, 0.5, axis=1)

    @testing.numpy_nlcpy_array_equal()
    def test_me_case_4(self, xp):
        a = xp.array([[10, 7, 4], [3, 2, 1]])
        return xp.quantile(a, 0.5, axis=1, keepdims=True)

    @testing.numpy_nlcpy_array_equal()
    def test_me_case_5(self, xp):
        a = xp.array([[10, 7, 4], [3, 2, 1]])
        m = xp.quantile(a, 0.5, axis=0)
        out = xp.zeros_like(m)
        return xp.quantile(a, 0.5, axis=0, out=out)

    @testing.numpy_nlcpy_array_equal()
    def test_me_case_6(self, xp):
        a = xp.array([[10, 7, 4], [3, 2, 1]])
        b = a.copy()
        return xp.quantile(b, 0.5, axis=1, overwrite_input=True)
